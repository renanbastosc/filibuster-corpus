...
def test_functional_cinema_3_user_user_bookings():
    ...
    if not was_fault_injected():
        assert users_bookings.status_code == 200
        assert users_bookings.json() == {'20151201': [{'rating': 8.8,'title': 'Creed','uri': '/movies/267eedb8-0f5d-42d5-8f43-72426b9fb3e6'}]}
    else:
        assert was_fault_injected() and users_bookings.status_code in [503, 404, 200]
        if users_bookings.status_code == 200:
            assert users_bookings.json() == {'20151201': [{'rating': 8.8,'title': 'Creed','uri': '/movies/267eedb8-0f5d-42d5-8f43-72426b9fb3e6'}]}

if __name__ == "__main__":
    test_functional_cinema_3_user_user_bookings()